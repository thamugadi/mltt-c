...
{
  FAIL_IF_NOT(CAST_NAME(type)->type.level < 65535, "!!type level overflow!!");
  return make_type(1 + CAST_NAME(type)->type.level);
error:
  return NULL;
}
...
{
  Expr* t;
  t = get_type(var_env, CAST_NAME(var)->var.index);
  shift(t, CAST_NAME(var)->var.index + 1, 0);
...
{
  Expr* type = get_type_hashmap(CAST_NAME(var)->def.name);
...
{
  Expr* type_annot = CAST_NAME(lam)->lam.type_annot;
...
{
  Expr* type_annot = CAST_NAME(pair)->pair.type_annot;
...
Expr* type_infer_app(Expr* app, var_env_t* var_env)
{
  ...
  f_type = type_infer(CAST_NAME(app)->app.left, var_env);
  FAIL_IF_NOT(f_type, "todo:1001");
  FAIL_IF_NOT(CAST_NAME(f_type)->tag == PI, "todo:1002");

  bool res = type_check(CAST_NAME(app)->app.right, CAST_NAME(f_type)->pi.dom, var_env);
  FAIL_IF_NOT(res, "todo:1004");

  res_type = subst_top(CAST_NAME(f_type)->pi.cod, CAST_NAME(app)->app.right);
  free_expr(f_type);
...
Expr* type_infer_pi(Expr* pi, var_env_t* var_env)
{
  ...
  dom_type = type_infer(CAST_NAME(pi)->pi.dom, var_env);
  FAIL_IF_NOT(dom_type, "todo:msg_11");
  FAIL_IF_NOT(CAST_NAME(dom_type)->tag == TYPE, "todo:msg_12");

  normal_dom = normalize_1(CAST_NAME(pi)->pi.dom, var_env);
  FAIL_IF_NOT(normal_dom, "todo:msg_13");

  new_env = add_type(normal_dom, var_env);
  FAIL_IF_NOT(new_env, "todo:msg_14");

  cod_type = type_infer(CAST_NAME(pi)->pi.cod, new_env);
  FAIL_IF_NOT(cod_type, "todo:msg_15");
  FAIL_IF_NOT(CAST_NAME(cod_type)->tag == TYPE, "todo:msg_16");

  level_t lv = (CAST_NAME(dom_type)->type.level > CAST_NAME(cod_type)->type.level)
    ? CAST_NAME(dom_type)->type.level
    : CAST_NAME(cod_type)->type.level;
...
Expr* type_infer_sig(Expr* sig, var_env_t* var_env)
{
  ...
  fty_type = type_infer(CAST_NAME(sig)->sig.f_ty, var_env);
  FAIL_IF_NOT(fty_type, "todo:msg_17");
  FAIL_IF_NOT(CAST_NAME(fty_type)->tag == TYPE, "todo:msg_18");

  normal_fty = normalize_1(CAST_NAME(sig)->sig.f_ty, var_env);
  FAIL_IF_NOT(normal_fty, "todo:msg_19");

  new_env = add_type(normal_fty, var_env);
  FAIL_IF_NOT(new_env, "todo:msg_20");

  family_type = type_infer(CAST_NAME(sig)->sig.family, new_env);
  FAIL_IF_NOT(family_type, "todo:msg_21");
  FAIL_IF_NOT(CAST_NAME(family_type)->tag == TYPE, "todo:msg_22");

  level_t lv = (CAST_NAME(fty_type)->type.level > CAST_NAME(family_type)->type.level)
    ? CAST_NAME(fty_type)->type.level
    : CAST_NAME(family_type)->type.level;
  free_var_env_until(new_env, var_env);
  free_expr(fty_type);
...
Expr* type_infer_fst(Expr* expr, var_env_t* var_env)
{
  ...
  ty_e = type_infer(CAST_NAME(expr)->fst.expr, var_env);
  FAIL_IF_NOT(ty_e, "todo:msg_23");
  FAIL_IF_NOT(CAST_NAME(ty_e)->tag == SIG, "fst");

  f_ty = copy_expr(CAST_NAME(ty_e)->sig.f_ty);
...
Expr* type_infer_snd(Expr* expr, var_env_t* var_env)
{
  ...
  ty_e = type_infer(CAST_NAME(expr)->snd.expr, var_env);
  FAIL_IF_NOT(ty_e, "todo:msg_25");
  FAIL_IF_NOT(CAST_NAME(ty_e)->tag == SIG, "snd");

  fst_e = make_fst(CAST_NAME(expr)->snd.expr, true);
  ...
  family_applied = subst_top(CAST_NAME(ty_e)->sig.family, normal_fst);
...
Expr* type_infer_refl(Expr* refl, var_env_t* var_env)
{
  ...
  ty = copy_expr(CAST_NAME(refl)->refl.ty);
  FAIL_IF_NOT(ty, "todo:msg_31");

  a = copy_expr(CAST_NAME(refl)->refl.a);
...
Expr* type_infer_w(Expr* w, var_env_t* var_env)
{
  ...
  type_C = type_infer(CAST_NAME(w)->w.C, var_env);
  FAIL_IF_NOT(type_C, "todo:msg_36");
  FAIL_IF_NOT(CAST_NAME(type_C)->tag == TYPE, "todo:msg_37");

  normal_C = normalize_1(CAST_NAME(w)->w.C, var_env);
  new_env = add_type(normal_C, var_env);

  A_type = type_infer(CAST_NAME(w)->w.family, new_env);
  FAIL_IF_NOT(A_type, "todo:msg_50001");
  FAIL_IF_NOT(CAST_NAME(A_type)->tag == TYPE, "todo:msg_5000");
  level_t lv = (CAST_NAME(type_C)->type.level > CAST_NAME(A_type)->type.level) ? CAST_NAME(type_C)->type.level
    : CAST_NAME(A_type)->type.level;
  free_var_env_until(new_env, var_env);
  free_expr(type_C);
  free_expr(normal_C);
  free_expr(A_type);
...
Expr* type_infer_tree(Expr* tree, var_env_t* var_env)
{
  ...
  type_C = type_infer(CAST_NAME(tree)->tree.root, var_env);
  FAIL_IF_NOT(type_C, "todo:msg_45");

  subtr_type = type_infer(CAST_NAME(tree)->tree.subtr, var_env);
  FAIL_IF_NOT(subtr_type, "todo:msg_46");

  normal_C = normalize_1(type_C, var_env);
  FAIL_IF_NOT(normal_C, "todo:msg_47");
  FAIL_IF_NOT(CAST_NAME(subtr_type)->tag == PI, "todo:msg_48");
  FAIL_IF_NOT(CAST_NAME(CAST_NAME(subtr_type)->pi.cod)->tag == W, "todo:msg_49");
  free_expr(type_C);

  new_env = add_type(CAST_NAME(CAST_NAME(subtr_type)->pi.cod)->w.C, var_env);
  type_A = type_infer(CAST_NAME(CAST_NAME(subtr_type)->pi.cod)->w.family, new_env);
  free_var_env_until(new_env, var_env);

  FAIL_IF_NOT(type_A, "todo:msg_51");
  FAIL_IF_NOT(cmp_expr(normal_C, CAST_NAME(CAST_NAME(subtr_type)->pi.cod)->w.C), "todo:msg_52");
  free_expr(type_A);
  free_expr(normal_C);

  Aroot = make_app(CAST_NAME(CAST_NAME(subtr_type)->pi.cod)->w.family, CAST_NAME(tree)->tree.root, true);
  normal_Aroot = normalize_1(Aroot, var_env);
  free_expr(Aroot);
  FAIL_IF_NOT(cmp_expr(normal_Aroot, CAST_NAME(subtr_type)->pi.dom), "todo:msg_521");
  free_expr(normal_Aroot);

  res_type = copy_expr(CAST_NAME(subtr_type)->pi.cod);
...
{
  Expr* type = CAST_NAME(id)->id.ty;
  FAIL_IF_NOT(type_check(CAST_NAME(id)->id.a, type, var_env), "todo:msg_58");
  FAIL_IF_NOT(type_check(CAST_NAME(id)->id.b, type, var_env), "todo:msg_59");
...
{
  Expr* x_type = NULL;
  Expr* C_type = type_infer(CAST_NAME(ind0)->ind0.C, var_env);
  FAIL_IF_NOT(C_type, "todo:msg_60");
  FAIL_IF_NOT(CAST_NAME(C_type)->tag == PI, "todo:msg_61");

  FAIL_IF_NOT(CAST_NAME(CAST_NAME(C_type)->pi.dom)->tag == ZERO, "todo:msg_62");
  FAIL_IF_NOT(CAST_NAME(CAST_NAME(C_type)->pi.cod)->tag == TYPE, "todo:msg_63");
  free_expr(C_type);

  x_type = type_infer(CAST_NAME(ind0)->ind0.x, var_env);
  FAIL_IF_NOT(CAST_NAME(x_type)->tag == ZERO, "todo:msg_64");
  free_expr(x_type);

  Expr* app = make_app(CAST_NAME(ind0)->ind0.C, CAST_NAME(ind0)->ind0.x, true);
...
Expr* type_infer_ind2(Expr* ind2, var_env_t* var_env)
{
  ...
  C_type = type_infer(CAST_NAME(ind2)->ind2.C, var_env);
  FAIL_IF_NOT(C_type, "todo:msg_65");
  FAIL_IF_NOT(CAST_NAME(C_type)->tag == PI, "todo:msg_66");

  FAIL_IF_NOT(CAST_NAME(CAST_NAME(C_type)->pi.dom)->tag == TWO, "todo:msg_67");
  FAIL_IF_NOT(CAST_NAME(CAST_NAME(C_type)->pi.cod)->tag == TYPE, "todo:msg_68");

  x_type = type_infer(CAST_NAME(ind2)->ind2.x, var_env);

  C_false = make_app(copy_expr(CAST_NAME(ind2)->ind2.C), make_false(), false);
  ...
  y_type = type_infer(CAST_NAME(ind2)->ind2.y, var_env);

  C_true = make_app(copy_expr(CAST_NAME(ind2)->ind2.C), make_true(), false);
  ...
  z_type = type_infer(CAST_NAME(ind2)->ind2.z, var_env);
  FAIL_IF_NOT(CAST_NAME(z_type)->tag == TWO, "todo:msg_71");

  normal_dom = normalize_1(CAST_NAME(C_type)->pi.dom, var_env);
  FAIL_IF_NOT(cmp_expr(z_type, normal_dom), "todo:msg_ind2err_z");
  free_expr(normal_dom);
  free_expr(C_type);
  free_expr(z_type);

  C_z = make_app(CAST_NAME(ind2)->ind2.C, CAST_NAME(ind2)->ind2.z, true);
...
bool type_check(Expr* term, Expr* type, var_env_t* var_env)
{
  if (GET_TAG(term) == LAM)
  {
    if (CAST_NAME(type)->tag != PI)
    {
      return false;
    }
    Expr* dom = CAST_NAME(type)->pi.dom;
    Expr* cod = CAST_NAME(type)->pi.cod;
    Expr* normal_dom = normalize(dom, var_env);
    var_env_t* new_env = add_type(normal_dom, var_env);

    Expr* normal_cod = normalize(cod, new_env);

    bool res = type_check(CAST_NAME(term)->lam.expr, normal_cod, new_env);
  ...
  {
    if (CAST_NAME(type)->tag != SIG)
    {
      return false;
    }
    Expr* f_ty = CAST_NAME(type)->sig.f_ty;
    Expr* family = CAST_NAME(type)->sig.family;
    Expr* normal_f_ty = normalize(f_ty, var_env);
    Expr* normal_family_app;
    if (!type_check(CAST_NAME(term)->pair.a, normal_f_ty, var_env))
    {
      free_expr(normal_f_ty);
      return false;
    }
    Expr* family_app = subst_top(family, CAST_NAME(term)->pair.a);
    normal_family_app = normalize_1(family_app, var_env);

    bool res = type_check(CAST_NAME(term)->pair.b, normal_family_app, var_env);
  ...
  {
    if (CAST_NAME(type)->tag == TYPE && CAST_NAME(type)->type.level == CAST_NAME(term)->type.level + 1)
    {
      return true;
    }
    return false;
